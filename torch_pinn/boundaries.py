"""
Boundary condition classes for PINN.
Faithful port from tensordiffeq/boundaries.py (TensorFlow).
"""

import numpy as np
from .utils import multimesh, flatten_and_stack, to_torch
...
class dirichletBC(BC):
    """Dirichlet boundary condition with fixed value."""
    ...
    def __init__(self, domain, val, var, target):
        ...
        self.target_val = self.dict_[var + target]
        self.compile()
        self.isDirichlet = True

    def create_input(self):
        repeated_value = self.create_target_input_repeat(self.var, self.target_val)
...
class FunctionDirichletBC(BC):
    """Dirichlet BC where target is a function."""

    def __init__(self, domain, fun, var, target, n_values=None):
        self.domain = domain
        self.fun = fun
        self.var = var
        self.target = target
    ...
    def create_input(self):
        ...
        dim_repeat = self.create_target_input_repeat(self.var, self.target_val)
    ...
    def create_target(self):
        ...
        if self.n_values is not None:
            self.val = np.reshape(fun_vals, (-1, 1))[self.nums]
        else:
            self.val = np.reshape(fun_vals, (-1, 1))


class FunctionNeumannBC(BC):
    """Neumann boundary condition with derivative model."""
