...
class CollocationSolverND:
    ...
    def compile(self, layer_sizes, f_model_list, domain, bcs, isAdaptive=False,
        ...
        Returns:
            None
        """
        self.tf_optimizer = tf.keras.optimizers.legacy.Adam(lr=0.001, beta_1=.99)
        self.tf_optimizer_2 = tf.keras.optimizers.legacy.Adam(lr=0.001, beta_1=.99)
        self.tf_optimizer_weights = tf.keras.optimizers.legacy.Adam(lr=0.0005, beta_1=.99)
    ...
    def fit(self, tf_iter=0, newton_iter=0, batch_sz=None, newton_eager=True):
        ...
        if self.dist:
            ...
            with self.strategy.scope():
                self.u_model = neural_net(self.layer_sizes)
                self.tf_optimizer = tf.keras.optimizers.legacy.Adam(lr=0.005, beta_1=.99)
                self.tf_optimizer_weights = tf.keras.optimizers.legacy.Adam(lr=0.005, beta_1=.99)
